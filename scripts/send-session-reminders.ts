...
async function sendSessionReminders() {
  ...
  console.log(`Found ${sessionsFor24hReminder.length} sessions for 24h reminder`)
  console.log(`Found ${sessionsFor1hReminder.length} sessions for 1h reminder`)
...
async function send24HourReminder(session: ReminderSession) {
  ...
  if (!phone) {
    console.log(`⚠️  No phone number for session ${session.id}`)
    return
  }
  ...
  console.log(`📱 Sending 24h reminder to ${phone}:`, message)
...
async function send1HourReminder(session: ReminderSession) {
  ...
  console.log(`📱 Sending 1h reminder to ${phone}:`, message)
